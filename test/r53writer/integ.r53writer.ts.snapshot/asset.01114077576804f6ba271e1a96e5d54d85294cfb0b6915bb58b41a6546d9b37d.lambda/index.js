...
var require_validations = __commonJS({
  "node_modules/electrodb/src/validations.js"(exports, module2) {
    ...
    var Validator = require_lib5().Validator;
    ...
    var Attribute = class _Attribute {
        ...
        if (typeof definition === "function") {
          return (val2) => {
            try {
              ...
              const isValid2 = !reason;
              if (isValid2) {
                return [isValid2, []];
              } else if (typeof reason === "boolean") {
                return [isValid2, [new e.ElectroUserValidationError(this.path, "Invalid value provided")]];
              } else {
                return [isValid2, [new e.ElectroUserValidationError(this.path, reason)]];
              }
            } catch (err) {
      ...
      isValid(value) {
        try {
          ...
          let [isValid2, validationError] = isTyped ? this.validate(value) : [false, []];
          let errors = [...typeErrorReason, ...validationError].filter((value2) => value2 !== void 0);
          return [isTyped && isValid2, errors];
        } catch (err) {
          return [false, [err]];
        }
      }
      ...
      getValidate(value) {
        ...
        let [isValid2, validationErrors] = this.isValid(value);
        if (!isValid2) {
      ...
      _validateChildren(value) {
        ...
        if (valueType === ValueTypes.object) {
          for (const child of Object.keys(attributes2)) {
            const [isValid2, errorValues] = attributes2[child].isValid(value === void 0 ? value : value[child]);
            if (!isValid2) {
      ...
      _validateChildren(value) {
        ...
        if (valueType === ValueTypes.array) {
          for (const i in value) {
            const [isValid2, errorValues] = this.items.isValid(value[i]);
            if (!isValid2) {
    ...
    var SetAttribute = class extends Attribute {
      ...
      _validateChildren(value) {
        ...
        for (const item of arr) {
          const [isValid2, errorValues] = this.items.isValid(item);
          if (!isValid2) {
            errors = [...errors, ...errorValues];
          }
        }
...
var require_client = __commonJS({
  "node_modules/electrodb/src/client.js"(exports, module2) {
    ...
    var util2 = require("@aws-sdk/lib-dynamodb/dist-cjs/commands/utils");
    ...
    var unmarshallOutput = util2.unmarshallOutput || ((val2) => val2);
    var v3Methods = ["send"];
...
var require_entity = __commonJS({
  "node_modules/electrodb/src/entity.js"(exports, module2) {
    ...
    var { EventManager } = require_events2();
      ...
      _normalizeExecutionOptions({ provided = [], context = {} } = {}) {
        ...
        return provided.filter(Boolean).reduce((config2, option) => {
          ...
          if (option.formatCursor) {
            const isValid2 = ["serialize", "deserialize"].every(
...
// src/Route53Writer/gc.lambda.ts
var gc_lambda_exports = {};
__export(gc_lambda_exports, {
  handler: () => handler
});
module.exports = __toCommonJS(gc_lambda_exports);
...
// src/Route53Writer/gc.lambda.ts
var CfnRegisteredRecord$ = z.object({
...
var handler = AppHandler(async (event) => {
...
async function gcRecords(cfnRecords) {
  ...
  await Promise.all(
    dbRecords.map(async (dnsRecord) => {
      ...
      const cfnRecord = cfnRecords.find((cfnRecord2) => {
        return cfnRecord2.HostedZoneId === dnsRecord.hostedZoneId && cfnRecord2.Name === dnsRecord.name;
      });
      if (cfnRecord) {
      ...
      await database.entities.DnsRecord.delete(dnsRecord).go();
    })
  );
}
  ...
  await Promise.all(
    dbRecords.map(async (dnsRecord) => {
      try {
        ...
        await database.entities.DnsRecord.delete(dnsRecord).go();
        const deadline = Date.now() + 30 * 1e3;
        let recordSet = await getRecordSet(dnsRecord);
        while (recordSet !== void 0) {
